...
U-TOPIA는 프리미엄 VOD와 크리에이터 콘텐츠를 통합 제공하는 웹 기반 OTT 플랫폼입니다. 관리자가 업로드하는 OTT 콘텐츠와 크리에이터가 직접 제작하는 유저 콘텐츠를 모두 지원하며, 개인화 추천과 실시간 인기 차트를 통해 사용자 맞춤형 경험을 제공합니다.
...
- 구독 등급별 기능 제한 (일반: 720p·배속 잠금 / 구독·LG U+: 전체 화질·배속)
- 로그인 실패 횟수 표시 및 5회 초과 시 계정 잠금 안내 (15분)
- 이어보기 (10초 간격 자동 저장)
- 찜하기 / 시청 이력 관리
- 댓글 CRUD
...
- SSE 기반 트랜스코딩 완료 감지 + 폴링 폴백 (자동 활성화)
...
- SSE + 폴링 기반 트랜스코딩 완료 감지 및 자동 활성화
- 썸네일 업로드
- 콘텐츠 제목 검색 (클라이언트 필터링)
- 사용자 관리
- 홈 태그 통계 (Recharts 차트)
...
| sseService          | 2      | 트랜스코딩 완료 실시간 감지 (Admin/Creator)   |
...
src/
├── components/
│   ├── common/          # Header, Footer, VideoPlayer, AlertModal, ConfirmModal
